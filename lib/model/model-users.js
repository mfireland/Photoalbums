...
function getAllUsers(callback) {
  connection.query('SELECT username, userID FROM users', (err, rows, fields) => {
    if (err) {
      callback(err);
    } else {
      callback(null, rows);
...
function getUser(params, callback) {
  ...
  connection.query('SELECT username, userID FROM users WHERE username=' + connection.escape(params.username), (err, rows, fields) => {
    if (err) {
...
function loginUser(params, callback) {
  ...
  connection.query('SELECT username, password, userID FROM users WHERE username=' + connection.escape(params.username), (err, rows, fields) => {
    if (err) {
